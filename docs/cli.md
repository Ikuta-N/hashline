...
**空白や `#` を含む本文は引用符で囲むこと。** `#` はシェルではコメントの
開始なので、`hashline #日記 晴れ` はシェルの段階で `hashline` だけになり、
ノートを書かずに Web UI が立ち上がる。
...
ファイルシステムを直接読み、認証も無いため、外向けに開かない。ループバック
以外の `--host` を渡すと、起動時にその旨を警告する。

`--reload` が見るのは hashline 自身のソースであって、コマンドを実行した
ディレクトリではない。開発中でなければ使う必要はない。
